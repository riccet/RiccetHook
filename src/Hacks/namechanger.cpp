...
void NameChanger::BeginFrame(float frameTime)
{
	...
	if (changes >= 5)
	{
		switch (NameChanger::type)
		{
			case NC_Type::NC_NORMAL:
				SetName(Util::PadStringRight(XORSTR("\230HITLERHOOK"), strlen(XORSTR("\230HITLERHOOK")) + Util::RandomInt(10, 50)));
				break;
	...
	SetName(Util::PadStringRight(XORSTR("HITLERHOOK"), strlen(XORSTR("HITLERHOOK")) + changes));
}